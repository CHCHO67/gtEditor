...
## Product goals
- Goals: let reviewers process one table at a time, classify it as Accepted Origin, Revision, or Discard, and persist every decision to val-compatible Output_data buckets.
...
- Core routes/screens: active dataset tab, sub-tabs `Needs Review`, `Accepted Origin`, `Revision`, `Discard`, table canvas, status/warnings pane.
...
- Variants and states: needs-review/accepted-origin/revision/discard list states; blue selected cells; purple merged cells; green source text boxes; disabled action state when no active file.
...
- Success: status bar and warnings pane show accepted/revision/discard output path/counts, with files under `accepted_origin/image,json`, `revision/image,json`, or `discarded/image,json`.
...
- Terminology: use `Input_data`, `Output_data`, `Needs Review`, `Accepted Origin`, `Revision`, `Discard`, `Save`, `Discard` consistently.
...
- Compatibility constraints: keep CLI `--save-all` and existing IO helper contracts for tests/automation; saved review JSON must remain table_structure_val-compatible with canonical key order, float `layout_tedss_score`, no trailing newline, crop-local table bounds, and no empty-text placeholder cells.
